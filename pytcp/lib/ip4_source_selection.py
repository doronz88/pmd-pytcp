...
# Scope codepoints — same integer space as the IPv6 helper so
# rule-2 comparison stays a plain integer compare across
# families. Numeric values mirror RFC 4007 §5 / RFC 4291 §2.7.
IP4__SCOPE__INTERFACE_LOCAL: int = 0x1
IP4__SCOPE__LINK_LOCAL: int = 0x2
IP4__SCOPE__GLOBAL: int = 0xE


def ip4_address_scope(address: Ip4Address, /) -> int:
    """
    ...
    if address.is_loopback:
        return IP4__SCOPE__INTERFACE_LOCAL
    if address.is_link_local:
        return IP4__SCOPE__LINK_LOCAL
    return IP4__SCOPE__GLOBAL
