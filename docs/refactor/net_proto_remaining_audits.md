...
| Audits A–G (roundtrip, base, errors, lib, enums, prop-docstrings, §7.2 test-docstring sweep) | 2026-05-21 | A/E confirmed via git; B/C/D/F prior session; G fully closed (see `docs/refactor/archive/audit_g_test_docstring_sweep.md`) |
...
| Audit K — `Buffer` alias adoption | 2026-05-21 | commit `0e236720` — fixed 3 `list[bytes \| bytearray \| memoryview]` → `list[Buffer]` in `test__ip6__rfc6437_flow_label.py`. The 9 net_addr value-type ctor signatures correctly keep the spelled-out union (net_addr sits below net_proto; can't import the alias — `net_addr.md` §1). |
| Audit L — `*Options`/`*OptionsProperties` layout | 2026-05-21 | commits `b493f828` (TCP `_options` reach-through→public `options`), `7ab37061` (DHCPv4 `srv_id`→`server_id`), `3640a36c` (IPv4 lookup props mixin→`Ip4Options` container), `368134ad` (ICMPv6-ND `option_*`→bare `slla`/`tlla`/`pi`/`nonce`), `7321497c` (black re-wrap). **Target-A reached:** raw lookup props now live on the `*Options` container in every family; holder accessors use bare option names. HBH/DestOpts deliberately left container-only (no consumer needs a holder accessor — that's absence of surface, not drift). TCP mixin keeps its defaulted-subset surface (intentional, host-FSM-relevant). 11158 tests passing. |
