...
- **For the no-GIL north star:** F2, F3, and F4 were all real
  ...
  query-response timer state (M1), and the IGMP state-change
  retransmit-timer compat-mode read/RMW (M2) — all SHIPPED
  2026-05-27. Still unguarded: `TcpSession` timer/CC state,
  per-interface address-config + multicast-membership lists, the
  ICMP error rate-limiters, the per-socket source filter, and the
