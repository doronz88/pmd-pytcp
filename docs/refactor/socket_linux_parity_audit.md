...
- **For the no-GIL north star:** F2, F3, and F4 were all real
  ...
  retransmit-timer compat-mode read/RMW (M2), and the ICMP error
  rate-limiter token bucket (I1) — all SHIPPED 2026-05-27. Still
  unguarded: `TcpSession` timer/CC state, per-interface
  address-config + multicast-membership lists, the per-socket
  source filter, and the `PacketStats` counters. The
  authoritative no-GIL ledger + correction plan is now
