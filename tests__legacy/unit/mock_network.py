...
def setup_mock_packet_handler(self: TestCase) -> None:
    ...
    def _mock_enqueue(packet_tx: memoryview) -> None:
        """
        Mock 'TxRing.enqueue()' method to record the assembled frames.
        """

        self.frame_tx = bytes(packet_tx)
        self.frames_tx.append(self.frame_tx)
    ...
    self.mock_callable(
        ...
        ip4_address=self.mns.host_a_ip4_address
    ).to_return_value(self.mns.host_a_mac_address)
    self.mock_callable(
        target=mock_ArpCache,
        method="find_entry",
    ).for_call(
        ip4_address=self.mns.host_b_ip4_address
    ...
    self.mock_callable(
        ...
        ip6_address=self.mns.host_a_ip6_address
    ).to_return_value(self.mns.host_a_mac_address)
    self.mock_callable(
        target=mock_NdCache,
        method="find_entry",
    ).for_call(
        ip6_address=self.mns.host_b_ip6_address
    ).to_return_value(None)
    self.mock_callable(
        target=mock_NdCache,
        method="find_entry",
    ).for_call(
        ip6_address=self.mns.stack_ip6_gateway
