...
NetAddr package: importing 'pmd_net_addr' must not pull in 'click', yet the
'ClickType*' names must still resolve on first access.

pmd_net_addr/tests/unit/test__lazy_click.py
...
class TestNetAddrLazyClick(TestCase):
    ...
    def test__net_addr__import_does_not_pull_click(self) -> None:
        """
        Ensure 'import pmd_net_addr' does not import 'click' nor the
        'pmd_net_addr.click_types' module, so the base install stays
        stdlib-only.

        Reference: PyTCP test infrastructure (no RFC clause).
        """

        result = _run("import sys, pmd_net_addr; print('click' in sys.modules, 'pmd_net_addr.click_types' in sys.modules)")
        ...
        self.assertEqual(
            result.stdout.strip(),
            "False False",
            msg="'import pmd_net_addr' must not pull in 'click' or 'pmd_net_addr.click_types'.",
        )

    def test__net_addr__click_type_resolves_to_real_class(self) -> None:
        """
        Ensure a lazily-exposed 'ClickType*' name resolves to the
        exact class defined in 'pmd_net_addr.click_types'.
        ...
        result = _run(
            "import pmd_net_addr\n"
            "from pmd_net_addr.click_types import ClickTypeIp4Address as Real\n"
            "print(pmd_net_addr.ClickTypeIp4Address is Real)"
        )
        ...
        self.assertEqual(
            result.stdout.strip(),
            "True",
            msg="pmd_net_addr.ClickTypeIp4Address must be the class from pmd_net_addr.click_types.",
        )
    ...
    def test__net_addr__click_type_access_triggers_click_import(self) -> None:
        ...
        result = _run(
            "import sys, pmd_net_addr\n"
            "before = 'click' in sys.modules\n"
            "pmd_net_addr.ClickTypeIp4Address\n"
    ...
    def test__net_addr__unknown_attribute_raises_attribute_error(self) -> None:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "ATTRIBUTE_ERROR",
            msg="Accessing an unknown pmd_net_addr attribute must raise AttributeError.",
        )
