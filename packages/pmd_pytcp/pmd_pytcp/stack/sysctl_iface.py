...
knobs. Pairs with 'pmd_pytcp.stack.sysctl' (the operator-facing
...
pmd_pytcp/stack/sysctl_iface.py
...
from pmd_pytcp.stack.sysctl import _registry
...
def _resolve_iface_knob(base_key: str) -> Any:
    ...
    if not knob.interface_scope:
        raise KeyError(
            f"sysctl {base_key!r} is not interface-scope; " f"use 'pmd_pytcp.stack.sysctl.get'/'set' for flat keys",
        )
    return knob
