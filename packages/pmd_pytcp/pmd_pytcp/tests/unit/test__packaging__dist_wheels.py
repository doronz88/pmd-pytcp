...
"""
This module contains packaging tests asserting the built PyTCP-pmd_net_proto
and PyTCP-pmd_net_addr wheels ship their full package payload — in particular
that pmd_net_proto's subpackages (every protocol family + options) are
...
pmd_pytcp/tests/unit/test__packaging__dist_wheels.py
...
_PKG_NET_PROTO = _REPO_ROOT / "packages" / "pmd_net_proto"
_PKG_NET_ADDR = _REPO_ROOT / "packages" / "pmd_net_addr"
_PKG_PYTCP = _REPO_ROOT / "packages" / "pmd_pytcp"
...
class TestPackagingNetProtoWheel(TestCase):
    """
    The PyTCP-pmd_net_proto wheel-payload packaging tests.
    """
    ...
    def setUpClass(cls) -> None:
        """
        Build the pmd_net_proto wheel once for the whole class
    ...
    def test__net_proto__ships_subpackages(self) -> None:
        """
        Ensure the pmd_net_proto wheel ships every protocol-family
        subpackage (pmd_net_proto.protocols.*), not just the
        ...
        for prefix in (
            "pmd_net_proto/protocols/tcp/",
            "pmd_net_proto/protocols/ip6/",
            "pmd_net_proto/protocols/icmp6/",
            "pmd_net_proto/protocols/arp/",
            "pmd_net_proto/protocols/udp/",
            "pmd_net_proto/lib/",
        ):
            with self.subTest(prefix=prefix):
                self.assertTrue(
                    any(p.startswith(prefix) for p in self._payload),
                    msg=f"pmd_net_proto wheel must ship the {prefix!r} subpackage; payload is missing it.",
                )

    def test__net_proto__excludes_tests_and_ships_py_typed(self) -> None:
        """
        Ensure the pmd_net_proto wheel excludes the test tree and
        ...
        self.assertNotIn(
            True,
            {p.startswith("pmd_net_proto/tests/") for p in self._payload},
            msg="pmd_net_proto wheel must NOT ship the test tree.",
        )
        self.assertIn(
            "@py.typed",
            self._payload,
            msg="pmd_net_proto wheel must ship 'py.typed' (PEP 561).",
        )


class TestPackagingNetAddrWheel(TestCase):
    """
    The PyTCP-pmd_net_addr wheel-payload regression tests.
    """
    ...
    def setUpClass(cls) -> None:
        """
        Build the pmd_net_addr wheel once for the whole class.
        """
    ...
    def test__net_addr__ships_modules_excludes_tests_with_py_typed(self) -> None:
        """
        Ensure the pmd_net_addr wheel still ships its modules, omits
        the test tree, and carries 'py.typed' — locks the
        already-shipped PyTCP-pmd_net_addr layout against regression.
        ...
        self.assertIn(
            ...
            self._payload,
            msg="pmd_net_addr wheel must ship its value-type modules.",
        )
        self.assertNotIn(
            True,
            {p.startswith("pmd_net_addr/tests/") for p in self._payload},
            msg="pmd_net_addr wheel must NOT ship the test tree.",
        )
        self.assertIn(
            "@py.typed",
            self._payload,
            msg="pmd_net_addr wheel must ship 'py.typed' (PEP 561).",
        )


class TestPackagingPytcpWheel(TestCase):
    """
    The PyTCP (umbrella -> pmd_pytcp) wheel-payload packaging tests.
    """
    ...
    def setUpClass(cls) -> None:
        """
        Build the pmd_pytcp wheel once for the whole class.
        """
    ...
    def test__pytcp__ships_subpackages(self) -> None:
        ...
        subpackage (pmd_pytcp.runtime.*, pmd_pytcp.protocols.*) — the
        exact defect that left the historical umbrella wheel
        non-functional; this is the umbrella fix realised by
        dissolving it into the per-package pmd_pytcp dist.

        Reference: PyTCP test infrastructure (no RFC clause).
        """

        for prefix in (
            "pmd_pytcp/runtime/packet_handler/",
            "pmd_pytcp/protocols/tcp/",
            "pmd_pytcp/protocols/ip6/",
            "pmd_pytcp/socket/",
            "pmd_pytcp/stack/",
            "pmd_pytcp/lib/",
        ):
    ...
    def test__pytcp__excludes_tests_and_ships_py_typed(self) -> None:
        ...
        self.assertNotIn(
            True,
            {p.startswith("pmd_pytcp/tests/") for p in self._payload},
