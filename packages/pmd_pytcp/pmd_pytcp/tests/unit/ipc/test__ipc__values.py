...
pmd_pytcp/tests/unit/ipc/test__ipc__values.py
...
from pmd_pytcp.ipc.ipc__errors import IpcValueError
...
class TestIpcValuesRoundTrip(TestCase):
    ...
    def test__ipc__values__net_addr_types(self) -> None:
        """
        Ensure each pmd_net_addr value type round-trips to an equal value
        ...
        for value in [
            ...
            with self.subTest(value=value):
                self.assertEqual(
                    decode_value(encode_value(value)),
                    value,
                    msg=f"pmd_net_addr value {value!r} must round-trip to an equal value.",
                )
    ...
    def test__ipc__values__socket_option_enums_round_trip(self) -> None:
        ...
        pmd_net_proto ProtoEnum, which is not equal to its integer) survives
