...
'pmd_pytcp/protocols/dhcp6/dhcp6__client.py'.

pmd_pytcp/tests/unit/protocols/dhcp6/test__dhcp6__client.py
...
from pmd_pytcp.protocols.dhcp6.dhcp6__client import (
    Dhcp6Client,
    Dhcp6Lease,
    Dhcp6StatelessConfig,
)
from pmd_pytcp.protocols.dhcp6.dhcp6__uid import get_client_duid
from pmd_pytcp.socket import SO_BINDTODEVICE, SOL_SOCKET
...
class TestDhcp6ClientFetch(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._socket_factory = self.enterContext(
            patch("pmd_pytcp.protocols.dhcp6.dhcp6__client.socket", new=autospec_dhcp6_socket()),
        )
        self._sock = self._socket_factory.return_value

        self._random = self.enterContext(patch("pmd_pytcp.protocols.dhcp6.dhcp6__client.random"))
        self._random.randint.return_value = _PINNED_XID
        self._random.uniform.return_value = 0.0

        self.enterContext(patch("pmd_pytcp.protocols.dhcp6.dhcp6__client.log"))
        self.enterContext(patch("pmd_pytcp.runtime.subsystem.log"))
...
class TestDhcp6ClientLifecycle(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.enterContext(patch("pmd_pytcp.protocols.dhcp6.dhcp6__client.log"))
        self.enterContext(patch("pmd_pytcp.runtime.subsystem.log"))
        self.enterContext(patch("pmd_pytcp.protocols.dhcp6.dhcp6__client.SUBSYSTEM_SLEEP_TIME__SEC", 0.0))
