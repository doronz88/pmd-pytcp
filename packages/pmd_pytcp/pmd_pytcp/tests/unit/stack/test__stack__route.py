...
pmd_pytcp/tests/unit/stack/test__stack__route.py
...
class TestRouteControlTypesPublicSurface(TestCase):
    ...
    def test__stack__route__control_types_reexported_from_stack(self) -> None:
        ...
        from the sanctioned 'pmd_pytcp.stack' surface and are the same
        objects as the runtime definitions, so a consumer never has to
        reach into the implementation-detail 'pmd_pytcp.runtime.fib'.

        Reference: PyTCP test infrastructure (no RFC clause).
        """

        import pmd_pytcp.stack as stack_pkg
        from pmd_pytcp.runtime.fib import RouteScope as RuntimeRouteScope

        self.assertIs(
            stack_pkg.Route,
            Route,
            msg="pmd_pytcp.stack.Route must be the runtime Route type.",
        )
        self.assertIs(
            stack_pkg.RouteProtocol,
            RouteProtocol,
            msg="pmd_pytcp.stack.RouteProtocol must be the runtime RouteProtocol enum.",
        )
        self.assertIs(
            stack_pkg.RouteScope,
            RuntimeRouteScope,
            msg="pmd_pytcp.stack.RouteScope must be the runtime RouteScope enum.",
        )
