...
class Icmp6MessageEchoReply(Icmp6Message):
    """
    ...
    def __buffer__(self, _: int) -> memoryview:
        """
        Get the ICMPv6 Echo Reply message as a memoryview.
        """
    ...
    def validate_sanity(self, *, ip6__hop: int, ip6__src: Ip6Address, ip6__dst: Ip6Address) -> None:
        ...
        # RFC 4443 §4.2 — Echo Reply 'code' field MUST be 0; any other value
        # is unassigned by IANA and must be rejected.
        if self.code.is_unknown:
            raise Icmp6SanityError(
                f"The 'code' field of the ICMPv6 Echo Reply message "
                f"must be one of {Icmp6EchoReplyCode.get_known_values()}. "
                f"Got: {int(self.code)}."
            )
    ...
    def from_buffer(cls, buffer: Buffer, /) -> Self:
        ...
        type_, code, cksum, id, seq = struct.unpack(ICMP6__ECHO_REPLY__STRUCT, buffer[:ICMP6__ECHO_REPLY__LEN])
