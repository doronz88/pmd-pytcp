...
from pmd_net_proto.protocols.ip6.ip6__header import IP6__PAYLOAD__MAX_LEN
...
class Icmp6MessageEchoRequest(Icmp6Message):
    ...
    def __post_init__(self) -> None:
        """
        Ensure integrity of the ICMPv6 Echo Request message fields.
        """
    ...
    def validate_sanity(self, *, ip6__hop: int, ip6__src: Ip6Address, ip6__dst: Ip6Address) -> None:
        """
        Ensure sanity of the ICMPv6 Echo Request message after parsing it.
        """

        # RFC 4443 §4.1 — Echo Request 'code' field MUST be 0; any other
        # value is unassigned by IANA and must be rejected.
        if self.code.is_unknown:
            raise Icmp6SanityError(
                f"The 'code' field of the ICMPv6 Echo Request message "
                f"must be one of {Icmp6EchoRequestCode.get_known_values()}. "
                f"Got: {int(self.code)}."
            )
    ...
    def validate_integrity(*, frame: Buffer, ip6__dlen: int) -> None:
        """
        Ensure integrity of the ICMPv6 Echo Request message before parsing it.
        """
    ...
    def from_buffer(cls, buffer: Buffer, /) -> Self:
        ...
        type_, code, cksum, id, seq = struct.unpack(ICMP6__ECHO_REQUEST__STRUCT, buffer[:ICMP6__ECHO_REQUEST__LEN])
