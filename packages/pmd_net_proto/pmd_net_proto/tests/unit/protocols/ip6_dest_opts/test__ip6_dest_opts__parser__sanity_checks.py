...
"""
This module contains the IPv6 Dest Opts parser sanity-check tests.

pmd_net_proto/tests/unit/protocols/ip6_dest_opts/test__ip6_dest_opts__parser__sanity_checks.py
...
from pmd_net_proto.protocols.ip6_dest_opts.ip6_dest_opts__errors import Ip6DestOptsSanityError
from pmd_net_proto.protocols.ip6_dest_opts.ip6_dest_opts__parser import Ip6DestOptsParser


class TestIp6DestOptsParserSanity(TestCase):
    """
    The IPv6 Dest Opts parser sanity-check tests.
    ...
    def test__ip6_dest_opts__parser__sanity__skip_action_passes(self) -> None:
        """
        ...
        parser = Ip6DestOptsParser(PacketRx(b"\x06\x00\x06\x04\x00\x00\x00\x00"))
        self.assertEqual(
    ...
    def test__ip6_dest_opts__parser__sanity__discard_action_raises(self) -> None:
        """
        Ensure an unrecognized option with action 01 (discard +
        no ICMP) raises 'Ip6DestOptsSanityError' with 'pointer=None' so
        ...
        with self.assertRaises(Ip6DestOptsSanityError) as ctx:
            Ip6DestOptsParser(PacketRx(b"\x06\x00\x45\x04\x00\x00\x00\x00"))
        self.assertIsNone(
            ctx.exception.pointer,
            msg="Action 01 must produce no ICMP pointer (silent discard).",
        )

    def test__ip6_dest_opts__parser__sanity__param_problem_action_raises_with_pointer(self) -> None:
        """
        Ensure an unrecognized option with action 10 (discard +
        Param Problem code 2) raises 'Ip6DestOptsSanityError' with the
        ...
        #   Bytes 4-7 : 00 00 00 00 -> data
        with self.assertRaises(Ip6DestOptsSanityError) as ctx:
            Ip6DestOptsParser(PacketRx(b"\x06\x00\x85\x04\x00\x00\x00\x00"))
        self.assertEqual(
            ctx.exception.pointer,
            0,
    ...
    def test__ip6_dest_opts__parser__sanity__error_message_carries_proto_tag(self) -> None:
        """
        Ensure the sanity-error message starts with the canonical
        '[SANITY ERROR][IPv6 Dest Opts]' prefix produced by the protocol's
        error subclass.
        ...
        with self.assertRaises(Ip6DestOptsSanityError) as ctx:
            Ip6DestOptsParser(PacketRx(b"\x06\x00\x85\x04\x00\x00\x00\x00"))
        self.assertIn(
            "[SANITY ERROR][IPv6 Dest Opts]",
            str(ctx.exception),
            msg="Ip6DestOptsSanityError must carry canonical [SANITY ERROR][IPv6 Dest Opts] prefix.",
        )
