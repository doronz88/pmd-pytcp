...
pmd_net_proto/tests/unit/protocols/icmp4/test__icmp4__message__echo_reply__asserts.py
...
class TestIcmp4MessageEchoReplyAsserts(TestCase):
    """
    The ICMPv4 Echo Reply message constructor argument assert tests.
    """
    ...
    def test__icmp4__message__echo_reply__code__not_Icmp4EchoReplyCode(self) -> None:
        """
        Ensure the ICMPv4 Echo Reply message constructor rejects a 'code'
        argument that is not an Icmp4EchoReplyCode.

        Reference: RFC 792 (ICMPv4 Echo Reply type 0 fields).
        """
    ...
    def test__icmp4__message__echo_reply__cksum__under_min(self) -> None:
        """
        Ensure the ICMPv4 Echo Reply message constructor rejects a 'cksum'
    ...
    def test__icmp4__message__echo_reply__id__under_min(self) -> None:
        """
        Ensure the ICMPv4 Echo Reply message constructor rejects an 'id'
        argument below the 16-bit unsigned minimum.
    ...
    def test__icmp4__message__echo_reply__seq__under_min(self) -> None:
        """
        Ensure the ICMPv4 Echo Reply message constructor rejects a 'seq'
        argument below the 16-bit unsigned minimum.
    ...
    def test__icmp4__message__echo_reply__data__not_bytes(self) -> None:
        """
        Ensure the ICMPv4 Echo Reply message constructor rejects a 'data'
    ...
    def test__icmp4__message__echo_reply__data_len__over_max(self) -> None:
        ...
        Echo Reply header (i.e. the frame would no longer fit in an IPv4
        payload).
    ...
    def test__icmp4__message__echo_reply__data_len__at_max_accepted(self) -> None:
        """
        Ensure the ICMPv4 Echo Reply message constructor accepts a 'data'
        argument whose length is exactly IP4__PAYLOAD__MAX_LEN - header.

        Reference: RFC 792 (ICMPv4 Echo Reply type 0 fields).
        """

        max_len = IP4__PAYLOAD__MAX_LEN - ICMP4__ECHO_REPLY__LEN
        self._kwargs["data"] = b"X" * max_len

        message = Icmp4MessageEchoReply(**self._kwargs)
    ...
    def test__icmp4__message__echo_reply__id_seq_bounds_accepted(self) -> None:
        """
        Ensure the ICMPv4 Echo Reply message constructor accepts 'id' and
        ...
        for field_name in ("id", "seq"):
            for value in (UINT_16__MIN, UINT_16__MAX):
                with self.subTest(field_name=field_name, value=value):
                    ...
                    message = Icmp4MessageEchoReply(**kwargs)
                    self.assertEqual(
...
class TestIcmp4MessageEchoReplyFromBufferAsserts(TestCase):
    """
    The ICMPv4 Echo Reply message 'from_buffer()' assert tests.
    """

    def test__icmp4__message__echo_reply__from_buffer__wrong_type(self) -> None:
        """
        Ensure 'Icmp4MessageEchoReply.from_buffer()' refuses to parse a
        buffer whose first byte (ICMPv4 'type') is not ECHO_REPLY.
        ...
        with self.assertRaises(AssertionError) as error:
            Icmp4MessageEchoReply.from_buffer(
                # ICMPv4 (wrong type for Echo Reply)
                #   Type     : 255 (Unknown)
                #   Code     : 0
    ...
    def test__icmp4__message__echo_reply__from_buffer__correct_type_accepted(self) -> None:
        """
        Ensure 'Icmp4MessageEchoReply.from_buffer()' accepts a buffer whose
        first byte is ECHO_REPLY (0) and returns a concrete reply message.

        Reference: RFC 792 (ICMPv4 Echo Reply type 0 fields).
        """

        message = Icmp4MessageEchoReply.from_buffer(
            # ICMPv4 Echo Reply (minimal, zero-length data)
            #   Type     : 0 (Echo Reply)
            #   Code     : 0 (Default)
            #   Checksum : 0x0000
            #   Id       : 0x0000
        ...
        self.assertIsInstance(
            message,
            Icmp4MessageEchoReply,
            msg="from_buffer() must return an Icmp4MessageEchoReply instance.",
        )
