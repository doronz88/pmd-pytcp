...
"""
This module contains tests for the IPv6 HBH PadN option.

pmd_net_proto/tests/unit/protocols/ip6_hbh/test__ip6_hbh__option__padn.py
...
class TestIp6HbhOptionPadN(TestCase):
    """
    The IPv6 HBH PadN option happy-path matrix.
    """
    ...
    def test__ip6_hbh__option__padn__len(self) -> None:
        """
        ...
        opt = Ip6HbhOptionPadN(**self._kwargs)
        self.assertEqual(
            len(opt),
            self._results["len"],
    ...
    def test__ip6_hbh__option__padn__bytes(self) -> None:
        """
    ...
    def test__ip6_hbh__option__padn__str(self) -> None:
        """
    ...
    def test__ip6_hbh__option__padn__from_buffer_roundtrip(self) -> None:
        """
        ...
        original = Ip6HbhOptionPadN(**self._kwargs)
        recovered = Ip6HbhOptionPadN.from_buffer(bytes(original))
        self.assertEqual(
            recovered,
            original,
...
class TestIp6HbhOptionPadNAsserts(TestCase):
    """
    The IPv6 HBH PadN option constructor and parser-guard tests.
    """

    def test__ip6_hbh__option__padn__rejects_oversize_data(self) -> None:
        """
        ...
        with self.assertRaises(AssertionError):
            Ip6HbhOptionPadN(data=b"\x00" * 254)

    def test__ip6_hbh__option__padn__type_field_is_padn(self) -> None:
        """
        Ensure the PadN option's 'type' field is the canonical
        Ip6HbhOptionType.PADN enum member, set by the dataclass
        default.

        Reference: RFC 8200 §4.2 (PadN option type 1).
        """

        opt = Ip6HbhOptionPadN(data=b"\x00\x00")
        self.assertIs(
            opt.type,
            Ip6HbhOptionType.PADN,
            msg="PadN option type must be Ip6HbhOptionType.PADN.",
        )

    def test__ip6_hbh__option__padn__from_buffer_rejects_wrong_type(self) -> None:
        """
        ...
        with self.assertRaises(AssertionError):
            ...
            Ip6HbhOptionPadN.from_buffer(b"\x00\x00")

    def test__ip6_hbh__option__padn__from_buffer_rejects_truncated(self) -> None:
        """
        ...
        # opt_data_len + IP6_HBH__OPTION__LEN = 6, but len(buffer) = 3.
        with self.assertRaises(Ip6HbhIntegrityError):
            Ip6HbhOptionPadN.from_buffer(b"\x01\x04\x00")
