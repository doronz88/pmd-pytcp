...
pmd_net_proto/tests/unit/lib/test__lib__proto_enum.py
...
from pmd_net_proto.lib.proto_enum import ProtoEnum, ProtoEnumByte, ProtoEnumWord
...
class TestNetProtoLibProtoEnumNoAenum(TestCase):
    ...
    def test__net_proto__lib__proto_enum__import_does_not_pull_aenum(self) -> None:
        """
        Ensure importing pmd_net_proto (hence proto_enum) does not import
        ...
        result = subprocess.run(
            [sys.executable, "-c", "import sys, pmd_net_proto; print('aenum' in sys.modules)"],
            capture_output=True,
            text=True,
            env=env,
            check=False,
        ...
        self.assertEqual(
            result.stdout.strip(),
            "False",
            msg="Importing pmd_net_proto must not pull in the 'aenum' dependency.",
        )
