...
class TestIcmp4MessageEchoRequestAssemblerAsserts(TestCase):
    """
    ...
    def test__icmp4__message__echo_request__code__not_Icmp4EchoRequestCode(
        ...
        with self.assertRaises(AssertionError) as error:
            Icmp4MessageEchoRequest(*self._args, **self._kwargs)

        self.assertEqual(
            str(error.exception),
            f"The 'code' field must be an Icmp4EchoRequestCode. " f"Got: {type(value)!r}",
        )
...
class TestIcmp4MessageEchoRequestParserAsserts(TestCase):
    """
