    ...
    [
        ...
        },
        {
            "_description": (
                ...
                "the assembler must fill the ICMPv4 checksum field in-place."
            ),
            "_kwargs": {
            ...
            "_results": {
                ...
                # ICMPv4 Echo Request wire frame emitted by __buffer__ (8 bytes):
                #   Byte  0     : 0x08       -> ICMPv4 type = Echo Request
                #   Byte  1     : 0x00       -> ICMPv4 code = 0
                #   Bytes 2-3   : 0xf7fd     -> checksum auto-computed by __buffer__ over
                #                              the 8-byte payload (ICMPv4 has no pseudo header)
                #   Bytes 4-5   : 0x0001     -> Echo Request identifier = 1
                #   Bytes 6-7   : 0x0001     -> Echo Request sequence number = 1
                "__bytes__": b"\x08\x00\xf7\xfd\x00\x01\x00\x01",
        ...
        },
        {
            "_description": (
                ...
                "the default (zero) pseudo-header sum; the assembler must fill the ICMPv6 "
                "checksum field in-place using pshdr_sum=0."
            ),
            "_kwargs": {
        ...
        {
            "_description": (
                "Raw packet carrying an ICMPv4 Echo Request whose checksum is already set "
                "to a non-zero value; the assembler must not recompute it."
            ),
            "_kwargs": {
            ...
            "_results": {
                ...
                #   Byte  0     : 0x08       -> ICMPv4 type = Echo Request
                #   Byte  1     : 0x00       -> ICMPv4 code = 0
                #   Bytes 2-3   : 0x1234     -> preset checksum (not zero, so __buffer__ does NOT overwrite)
        ...
        },
        {
            "_description": (
                ...
                "non-zero value; the assembler must not recompute it."
            ),
            "_kwargs": {
            ...
            "_results": {
                ...
                # ICMPv6 Echo Request wire frame passed through verbatim:
                #   Byte  0     : 0x80       -> ICMPv6 type = Echo Request (128)
                #   Byte  1     : 0x00       -> ICMPv6 code = 0
                #   Bytes 2-3   : 0xabcd     -> preset checksum (not zero, so __buffer__ does NOT overwrite)
                #   Bytes 4-5   : 0x0001     -> identifier = 1
                #   Bytes 6-7   : 0x0001     -> sequence number = 1
                "__bytes__": b"\x80\x00\xab\xcd\x00\x01\x00\x01",
        ...
        },
        {
            "_description": (
                ...
                "shorter than 4 bytes, the checksum-overwrite slot does not exist and the "
            ...
            "_results": {
                ...
                # 3-byte payload — no bytes 2:3 slice that equals b"\x00\x00", so __buffer__
...
class TestRawAssemblerPshdrSum(TestCase):
    ...
    def test__raw__assembler__icmp4__pshdr_sum_ignored(self) -> None:
        """
        Ensure 'pshdr_sum' is ignored for ICMPv4 (which has no pseudo
        header); the auto-computed ICMPv4 checksum must be identical
        whether 'pshdr_sum' is 0 or non-zero.
        """
        ...
        assembler.pshdr_sum = 0xABCD  # must not influence ICMPv4

        # ICMPv4 Echo Request wire frame — pshdr_sum is irrelevant:
        #   Byte  0     : 0x08       -> ICMPv4 type = Echo Request
        #   Byte  1     : 0x00       -> ICMPv4 code = 0
        #   Bytes 2-3   : 0xf7fd     -> checksum auto-computed over the 8-byte payload
        #   Bytes 4-5   : 0x0001     -> identifier = 1
        #   Bytes 6-7   : 0x0001     -> sequence number = 1
        self.assertEqual(
            bytes(assembler),
            b"\x08\x00\xf7\xfd\x00\x01\x00\x01",
            msg="ICMPv4 checksum must not depend on pshdr_sum.",
        )
