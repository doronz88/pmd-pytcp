...
class TcpOptionAccecn1(TcpOption):
    ...
    def __post_init__(self) -> None:
        """
        Ensure integrity of the TCP AccECN1 option fields.
        """
    ...
    def __buffer__(self, _: int) -> memoryview:
        """
        Get the TCP AccECN1 option as a memoryview.
        """
    ...
    def _validate_integrity(buffer: Buffer, /) -> None:
        ...
        if (value := buffer[1]) != TCP__OPTION__ACCECN1__LEN:
            raise TcpIntegrityError(
                f"The TCP AccECN1 option length value must be {TCP__OPTION__ACCECN1__LEN} bytes. Got: {value!r}"
            )
    ...
    def from_buffer(cls, buffer: Buffer, /) -> Self:
        """
        Initialize the TCP AccECN1 option from buffer.
        """
        ...
        ee1b = int.from_bytes(buffer[2:5], "big")
        eceb = int.from_bytes(buffer[5:8], "big")
        ee0b = int.from_bytes(buffer[8:11], "big")
